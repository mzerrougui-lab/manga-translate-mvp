...
def translate_openai_batch(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if r.status_code == 429:
                # 429 can be temporary rate limit OR quota/plan issue.
                # If it's quota-related, retries won't help much.
                try:
                    ...
                    if "insufficient_quota" in err_type or "insufficient_quota" in err_code or "insufficient_quota" in err_msg:
            ...
            if not isinstance(translations, list) or len(translations) != len(texts):
                raise ValueError(f"Bad JSON shape: expected list of {len(texts)}, got {type(translations)} of len {len(translations) if isinstance(translations, list) else '?'}")
    ...
    # Failure fallback: return original text with error tag
...
def translate_batch(
    ...
    For LibreTranslate: Translates one-by-one (free API limitation)
    """
    if not items:
        return items
    ...
    # OpenAI (sequential: one text zone at a time)
    if backend.startswith("OpenAI"):
        translated_texts: List[str] = []

        total = len(texts)
        for idx, t in enumerate(texts, start=1):
            print(f"[OpenAI] Translating item {idx}/{total}...")
            # One request per text to reduce token spikes.
            out = translate_openai_batch(
                [t],
                ocr_lang,
                target_lang,
                api_key=openai_key,
            ...
            tr = out[0] if out else t

            # If OpenAI is rate-limited or quota is missing, fall back to LibreTranslate
            if "[OpenAI failed:" in tr:
                try:
                    tr = translate_libre(t, source_lang=ocr_lang, target_lang=target_lang)
                except Exception:
                    tr = t

            translated_texts.append(tr)

            # Small pacing to avoid RPM limits
